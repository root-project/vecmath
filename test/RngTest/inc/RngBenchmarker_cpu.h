...
ScalarKernelFunc_t ScalarKernelFunc[] = {ScalarMRG32k3a, ScalarThreefry,
                                         ScalarPhilox};
...
VectorKernelFunc_t VectorKernelFunc[] = {VectorMRG32k3a, VectorThreefry,
                                         VectorPhilox};
...
StateKernelFunc_t StateKernelFunc[] = {StateMRG32k3a, StateThreefry,
                                       StatePhilox};
...
Real_t VSLMRG32k3a(int nsample, double &result) {
  return VSLRngTest(nsample, result, VSL_BRNG_MRG32K3A);
}
Real_t VSLMCG59(int nsample, double &result) {
  return VSLRngTest(nsample, result, VSL_BRNG_MCG59);
}
Real_t VSLMT19937(int nsample, double &result) {
  return VSLRngTest(nsample, result, VSL_BRNG_MT19937);
}
Real_t VSLSMT19937(int nsample, double &result) {
  return VSLRngTest(nsample, result, VSL_BRNG_SFMT19937);
}
Real_t VSLGFSR250(int nsample, double &result) {
  return VSLRngTest(nsample, result, VSL_BRNG_R250);
}
Real_t VSLSOBOL32(int nsample, double &result) {
  return VSLRngTest(nsample, result, VSL_BRNG_SOBOL);
}

// OMP
// Real_t OPENMPMRG(int nsample, double& result);

VSLKernelFunc_t VSLKernelFunc[] = {VSLMRG32k3a, VSLMCG59,   VSLMT19937,
                                   VSLSMT19937, VSLGFSR250, VSLSOBOL32};
#endif
