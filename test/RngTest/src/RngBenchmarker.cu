...
void RngBenchmarker::RunCuda() {
  ...
  // 2. Threefry:
  vecRng::Threefry<vecRng::ScalarBackend> *threefry =
      new vecRng::Threefry<vecRng::ScalarBackend>();
  vecRng::Threefry<vecRng::ScalarBackend>::State_t *statesThreefry_d = 0;
  cudaMalloc((void **)&statesThreefry_d,
             theNBlocks * theNThreads *
                 sizeof(vecRng::Threefry<vecRng::ScalarBackend>::State_t));
  threefry->Initialize(statesThreefry_d, theNBlocks * theNThreads);

  // Philox:
  vecRng::Philox<vecRng::ScalarBackend> *philox =
      new vecRng::Philox<vecRng::ScalarBackend>();
  vecRng::Philox<vecRng::ScalarBackend>::State_t *statesPhilox_d = 0;
  cudaMalloc((void **)&statesPhilox_d,
             theNBlocks * theNThreads *
                 sizeof(vecRng::Philox<vecRng::ScalarBackend>::State_t));
  ...
  for (int k = 0; k < kNumberRng + 2; ++k) {
    ...
    for (unsigned r = 0; r < fRepetition; ++r) {
      ...
      if (cudaEnabled) {
        ...
        // call CUDA kernel
        if (k == 0) {
          CudaMRG32k3a(statesMRG32k3a_d, result_d, fNSample, theNBlocks,
                       theNThreads);
        }
        if (k == 1) {
          CudaThreefry(statesThreefry_d, result_d, fNSample, theNBlocks,
                       theNThreads);
        }
        if (k == 2) {
          CudaPhilox(statesPhilox_d, result_d, fNSample, theNBlocks,
                     theNThreads);
        }

        if (k == 3) {
          CurandMRG32k3a(devStatesMRG32k3a, result_d, fNSample, theNBlocks,
                         theNThreads);
        }

        if (k == 4) {
          CurandPhilox(devStatesPhilox, result_d, fNSample, theNBlocks,
                       theNThreads);
        }
  ...
  for (int k = 0; k < kNumberRng + 2; ++k) {
    ...
    if (k == kNumberRng) {
      printf(" %s Nvidia   Time = %6.4f +- %6.4f msec Sum = %g\n",
             "CurandMRG32k3a", meanEventTime[k], sigmaEventTime[k],
             rngEvent[k]);
    }
    if (k == kNumberRng + 1) {
      printf(" %s Nvidia   Time = %6.4f +- %6.4f msec Sum = %g\n",
             "CurandPhilox  ", meanEventTime[k], sigmaEventTime[k],
             rngEvent[k]);
    }
  }
