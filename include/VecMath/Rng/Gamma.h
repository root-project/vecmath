...
if(alpha > 1.0){
 ...
 Double_v d = alpha-1.0/3.0;
    ...
    Double_v x = static_cast<DerivedT *>(this)-> template Gauss<BackendT>(0.0,1.0);
    Double_v f=1.0+c*x;
    Double_v v=f*f*f;
    while (v<=0.0){
                         ...
                         typename BackendT::Double_v beta)
 {
  ...
  int vsize = VectorSize<Double_v>();
   ...
   if(MaskFull(gtOne)) { // all alpha > 1
     ...
     Double_v c = 1.0/math::Sqrt(9.0*d);
     Double_v g = static_cast<DerivedT *>(this)-> template Gauss<BackendT>(0.0,1.0);
     Double_v z = c*g + 1.0;
     Double_v v = z*z*z;
     Mask<Double_v> good (v >= Double_v(0.0));

     if(MaskFull(good)) { // test for normal(0,1) < - sqrt(9 * alpha - 3) < -2.45
       Double_v u = static_cast<DerivedT *>(this)-> template Uniform<BackendT>();
    ...
    for(int i = 0 ; i < vsize ; ++i) {
      if(flag[i]==true) { //accepted
        ...
        if(flag2[i]==true) { //accepted
          ...
          flag_s=false;
        do {
//double f= static_cast<DerivedT *>(this)-> template Gauss<BackendT>(0.0,1.0);
//f=g[i];
 g[i] = static_cast<DerivedT *>(this)-> template Gauss<ScalarBackend>(0.,1.);
  z[i] = c[i]*g[i] + 1.0;
   v[i] = z[i]*z[i]*z[i];
     if(v[i]>=0.0) {
     //  Mask<Double_v> good (v >= Double_v(0.0));
          u[i] = static_cast<DerivedT *>(this)-> template Uniform<ScalarBackend>();
        //  pe[i]=math::Log(u[i]);
        //  po[i]=math::Log(v[i]);
...
return gamma;
 ...
 if(MaskEmpty(gtOne)) { // 0 < alpha < 1
  Double_v  gamma = Gamma<BackendT>(alpha+1,beta);
    ...
    //Gamma<ScalarBackend> may be specialized with the original implementation
  }
