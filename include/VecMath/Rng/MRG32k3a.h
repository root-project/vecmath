...
template <typename BackendT> struct RNG_traits<MRG32k3a<BackendT>> {
...
Real_t MRG32k3a<BackendT>::fSeed[MRG::vsize] = {12345., 12345., 12345.,
                                                12345., 12345., 12345.};
...
void MRG32k3a<VectorBackend>::SetStateAt(unsigned int i, State_s *state) {
  for (int j = 0; j < MRG::vsize; ++j)
    this->fState->fCg[j][i] = state->fCg[j];
}
...
VECCORE_FORCE_INLINE
VECCORE_ATT_HOST_DEVICE
MRG32k3a<VectorBackend>::State_s
MRG32k3a<VectorBackend>::GetStateAt(unsigned int i) {
  State_s state;
  for (int j = 0; j < MRG::vsize; ++j) {
...
template <typename BackendT> VECCORE_FORCE_INLINE VECCORE_ATT_HOST void MRG32k3a<BackendT>::SetNextStream() {
...
template <> VECCORE_FORCE_INLINE VECCORE_ATT_HOST void MRG32k3a<ScalarBackend>::SetNextStream() {
  for (int j = 0; j < MRG::vsize; ++j) {
...
template <typename BackendT> VECCORE_FORCE_INLINE VECCORE_ATT_HOST void MRG32k3a<BackendT>::SetNextSubstream() {
  for (size_t i = 0; i < VectorSize<Double_v>(); ++i) {
    for (int j = 0; j < MRG::vsize; ++j) {
...
template <> VECCORE_FORCE_INLINE VECCORE_ATT_HOST void MRG32k3a<ScalarBackend>::SetNextSubstream() {
  for (int j = 0; j < MRG::vsize; ++j) {
...
VECCORE_FORCE_INLINE
VECCORE_ATT_HOST
void MRG32k3a<ScalarBackend>::Initialize(State_s *states,
...
// Kernel to generate the next random number(s) with ReturnTypeBackendT (based
// on RngStream::U01d)
template <class BackendT>
...
MRG32k3a<BackendT>::Kernel(State_t &state) {
  ...
  // Combination
  return Blend((p1 > p2), (p1 - p2) * MRG::norm,
               (p1 - p2 + MRG::m1) * MRG::norm);
...
VECCORE_FORCE_INLINE
VECCORE_ATT_HOST
void MRG32k3a<BackendT>::AdvanceState(long e, long c) {
...
template <> VECCORE_FORCE_INLINE VECCORE_ATT_HOST void MRG32k3a<ScalarBackend>::AdvanceState(long e, long c) {
  double C1[3][3], C2[3][3];
...
template <class BackendT>
VECCORE_ATT_HOST
void MRG32k3a<BackendT>::TransitionVector(double C1[3][3], double C2[3][3],
                                          double e, double c) {
...
template <class BackendT>
VECCORE_ATT_HOST
double MRG32k3a<BackendT>::MultModM(double a, double s, double c, double m) {
  double v;
  long a1;
...
template <class BackendT>
VECCORE_ATT_HOST
void MRG32k3a<BackendT>::MatVecModM(const double A[3][3], const double s[3],
                                    double v[3], double m) {
  int i;
  // Necessary if v = s
  double x[3];
...
template <class BackendT>
VECCORE_ATT_HOST
void MRG32k3a<BackendT>::MatMatModM(const double A[3][3], const double B[3][3],
...
template <class BackendT>
VECCORE_ATT_HOST
void MRG32k3a<BackendT>::MatTwoPowModM(const double A[3][3], double B[3][3],
                                       double m, long e) {
  int i, j;
...
template <class BackendT>
VECCORE_ATT_HOST
void MRG32k3a<BackendT>::MatPowModM(const double A[3][3], double B[3][3],
                                    double m, long n) {
  int i, j;
  double W[3][3];
